...
class DBInventoryManager:
    ...
    def update_inventory(self, product_name, quantity, unit_cost, price, expiration_date, product_type=None, medicine_type=None):
        try:
            ...
            if product_type and medicine_type:
                sql = '''UPDATE products SET quantity = ?, unit_cost = ?, price = ?, expiration_date = ?, product_type = ?, medicine_type = ?, updated_date = datetime('now', 'localtime'), item_name = ? WHERE product_name = ?'''
                self.cursor.execute(sql, (quantity, unit_cost, price, expiration_date, product_type, medicine_type, product_name, product_name))
            else:
                sql = '''UPDATE products SET quantity = ?, unit_cost = ?, price = ?, expiration_date = ?, updated_date = datetime('now', 'localtime'), item_name = ? WHERE product_name = ?'''
                self.cursor.execute(sql, (quantity, unit_cost, price, expiration_date, product_name, product_name))
    ...
    def add_product_with_id(self, product_id, product_name, product_type, quantity, unit_cost, price, medicine_type, expiration_date):
        ...
        try:
            ...
            sql = '''INSERT INTO products (product_id, product_name, product_type, quantity, unit_cost, price, medicine_type, expiration_date, item_name, item_id, date_added, updated_date)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now', 'localtime'), datetime('now', 'localtime'))'''
            self.cursor.execute(sql, (product_id, product_name, product_type, quantity, unit_cost, price, medicine_type, expiration_date, product_name, product_id))
            self.conn.commit()
            self.sync_manager.sync_to_sheets()
    ...
    def get_available_ids(self):
        ...
        self.cursor.execute("""
            ...
            SELECT n FROM seq
            WHERE n NOT IN (SELECT item_id FROM products)
            AND n <= (SELECT COALESCE(MAX(item_id), 0) + 1 FROM products)
            ORDER BY n
        """)
    ...
    def get_next_available_id(self):
        ...
        if available_ids:
            ...
            self.cursor.execute("SELECT COALESCE(MAX(item_id), 0) + 1 FROM products")
    ...
    def delete_item(self, item_id):
        try:
            self.cursor.execute("DELETE FROM products WHERE item_id = ?", (item_id,))
            self.conn.commit()
            self.sync_manager.sync_to_sheets()
            return True
        except Exception as e:
